...
def joinScreenImages(path, cnt, image_size=(540,960,3), mirror_size=(360,640,3), RGB=False):#mirror_left, mirror_right, 
    ...
    # Path of the attention data
    attention_path = path.replace("RGB/camera/ClearNoon", "AM/AM-CPP-AttLoss-400x225")\
                    .replace("_RGB_camera_ClearNoon", "_AM-CPP-AttLoss-400x225")\
                    .replace("*", f"{SE.frameToStamp(cnt)}")\
                    .replace(".jpg", ".npz")

    full_attention = np.load(attention_path)["attention"]
    full_attention = cv2.resize(full_attention, (screen_width*3,screen_height))
    
    # Binarizar

    attention_max = np.max(full_attention)
    attention_threshold = attention_max * 0.1
    full_attention = np.uint8(full_attention > attention_threshold)*255
    ...
    try:
        ...
        if RGB:
            attention_image = full_attention[:, 0:screen_width]
    ...
    try:
        ...
        if RGB:
            attention_image = full_attention[:, screen_width:screen_width*2]
            attention_image_rgb = np.uint8(cm.jet(attention_image*256)*255)[:,:,:3]
    ...
    try:
        ...
        if RGB:
            attention_image = full_attention[:, screen_width*2:]
            attention_image_rgb = np.uint8(cm.jet(attention_image*256)*255)[:,:,:3]
            kernel = np.ones((5,5), dtype=np.uint8)
            dilated_attention = cv2.dilate(np.uint8(attention_image)*255, kernel, iterations=1) 
            attention_image_rgb[np.where(np.minimum(dilated_attention>0 , attention_image <1)>0)] = [64,0,0]
...
def createFrame(rgb_image, attention_image,video_frame, direction, speed, acceleration, steering):
    ...
    # frame = np.zeros((360,1920,3), dtype=np.uint8)

    # frame[:rgb_image.shape[0]] = rgb_image
    # print(rgb_image.shape, rgb_image.dtype)
    # print(rgb_image.min(), rgb_image.max())

    # print(video_frame.shape, video_frame.dtype)
    # print(video_frame.min(), video_frame.max())

    # import sys; sys.exit()

    # start = rgb_image.shape[0]+50
    # frame[start:start+attention_image.shape[0]] = attention_image
    # start+=attention_image.shape[0]
    # height = 1080-start
    # proportion = float(height)/float(1080)
    # resize_size = (int(1920*proportion),int(1080*proportion))
    # video_frame = cv2.resize(video_frame, resize_size)
    # half_width = video_frame.shape[1]//2
    # base_width =  1920//2-half_width
    # frame[start:start+video_frame.shape[0],base_width:base_width+video_frame.shape[1]]  = video_frame
...
def process_data(input_file, directions_file, total_frames, rgb_images_path, attention_images_path, framerate, scenario, original_scenario, user, town, route, base_user):
    ...
    output_path = f'{output_dir}/{user}_{town}_{route}_video_AM-CPP-AttLoss-400x225_bin.avi'
