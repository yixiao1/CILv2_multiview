...
class OutsideRouteLanesTest(Criterion):
    ...
    def __init__(self, actor, route, optional=False, name="OutsideRouteLanesTest", terminate_on_failure=False):
        """
        Constructor
        """
        super(OutsideRouteLanesTest, self).__init__(name, actor, 0, None, optional, terminate_on_failure=terminate_on_failure)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
...
class RouteCompletionTest(Criterion):
    ...
    def reset(self, new_route):
        ...
        self.finished_meters += self._accum_meters[self._current_index]
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            for index in range(self._current_index, min(self._current_index + self._wsize + 1, self._route_length)):
                ...
                if dot_ve_wp > 0:
                    ...
                    self._current_index = index
                    self._percentage_route_completed = 100.0 * float(self._accum_meters[self._current_index]+self.finished_meters) \
                        / float(self._accum_meters[-1]+self.finished_meters)
