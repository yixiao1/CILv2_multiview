...
class MapImage(object):
    ...
    def draw_map_image(carla_map, pixels_per_meter, precision=0.05):
        ...
        for waypoint in topology:
            ...
            for w in waypoints:
                ...
                l = w.get_left_lane()
                while l and l.lane_type != carla_env.LaneType.Driving:
                    if l.lane_type == carla_env.LaneType.Shoulder:
                        shoulder[0].append(l)
                    if l.lane_type == carla_env.LaneType.Parking:
                        parking[0].append(l)
                    if l.lane_type == carla_env.LaneType.Sidewalk:
                        sidewalk[0].append(l)
                    l = l.get_left_lane()
                # Classify lane types until there are no waypoints by going right
                r = w.get_right_lane()
                while r and r.lane_type != carla_env.LaneType.Driving:
                    if r.lane_type == carla_env.LaneType.Shoulder:
                        shoulder[1].append(r)
                    if r.lane_type == carla_env.LaneType.Parking:
                        parking[1].append(r)
                    if r.lane_type == carla_env.LaneType.Sidewalk:
                        sidewalk[1].append(r)
                    r = r.get_right_lane()
    ...
    def draw_lane_marking_single_side(lane_marking_yellow_broken_surface,
        ...
        previous_marking_type = carla_env.LaneMarkingType.NONE
        previous_marking_color = carla_env.LaneMarkingColor.Other
        current_lane_marking = carla_env.LaneMarkingType.NONE
        ...
        for markings in markings_list:
            if markings[1] == carla_env.LaneMarkingColor.White and markings[0] == carla_env.LaneMarkingType.Solid:
                MapImage.draw_line(lane_marking_white_solid_surface, markings[2], 1)
            elif markings[1] == carla_env.LaneMarkingColor.Yellow and markings[0] == carla_env.LaneMarkingType.Solid:
                MapImage.draw_line(lane_marking_yellow_solid_surface, markings[2], 1)
            elif markings[1] == carla_env.LaneMarkingColor.White and markings[0] == carla_env.LaneMarkingType.Broken:
                MapImage.draw_line(lane_marking_white_broken_surface, markings[2], 1)
            elif markings[1] == carla_env.LaneMarkingColor.Yellow and markings[0] == carla_env.LaneMarkingType.Broken:
    ...
    def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign, pixels_per_meter, world_offset):
        ...
        if lane_marking_type == carla_env.LaneMarkingType.Broken or (lane_marking_type == carla_env.LaneMarkingType.Solid):
            ...
            marking_2 = [
                MapImage.world_to_pixel(
                    ...
                    pixels_per_meter, world_offset) for w in waypoints]
            if lane_marking_type == carla_env.LaneMarkingType.SolidBroken:
                return [(carla_env.LaneMarkingType.Broken, lane_marking_color, marking_1),
                        (carla_env.LaneMarkingType.Solid, lane_marking_color, marking_2)]
            elif lane_marking_type == carla_env.LaneMarkingType.BrokenSolid:
                return [(carla_env.LaneMarkingType.Solid, lane_marking_color, marking_1),
                        (carla_env.LaneMarkingType.Broken, lane_marking_color, marking_2)]
            elif lane_marking_type == carla_env.LaneMarkingType.BrokenBroken:
                return [(carla_env.LaneMarkingType.Broken, lane_marking_color, marking_1),
                        (carla_env.LaneMarkingType.Broken, lane_marking_color, marking_2)]
            elif lane_marking_type == carla_env.LaneMarkingType.SolidSolid:
                return [(carla_env.LaneMarkingType.Solid, lane_marking_color, marking_1),
                        (carla_env.LaneMarkingType.Solid, lane_marking_color, marking_2)]
        return [(carla_env.LaneMarkingType.NONE, lane_marking_color, marking_1)]
...
if __name__ == '__main__':
    ...
    for cfg in server_manager.env_configs:
        ...
        client = carla_env.Client("localhost", cfg['port'])
