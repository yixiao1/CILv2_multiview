...
from network.models.architectures.Roach_rl_birdview.obs_manager import ObsManagerBase
from network.models.architectures.Roach_rl_birdview.utils.traffic_light import TrafficLightHandler
from network.models.architectures.Roach_rl_birdview import run_stop_sign
...
class ObsManager(ObsManagerBase):
    ...
    def get_observation(self):
        ...
        # In CARLA 0.9.14, carla.CityObjectLabel.Vehicles doesn't exist, so we account for this
        # re: https://github.com/carla-simulator/carla/issues/6216
        try:
            ...
            for vehicle_type in [
                ...
                vehicle_bbox_list.extend(self._world.get_level_bbs(vehicle_type))
        walker_bbox_list = self._world.get_level_bbs(carla.CityObjectLabel.Pedestrians)
        if self._scale_bbox:
            vehicles = self._get_surrounding_actors(vehicle_bbox_list, is_within_distance, 1.5)
            walkers = self._get_surrounding_actors(walker_bbox_list, is_within_distance, 6.0)
        else:
